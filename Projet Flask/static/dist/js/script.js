...
function getChatHistory() {
    const messageBlock = document.querySelectorAll(".message:not(#chat-template .message)");
    return Array.from(messageBlock).map(block => block.innerHTML)
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    form.addEventListener("submit", async(event) => {
        ...
        try {
            form.reset();
            const answerBlock = addToLog("L'ia est en train de réfléchir..");
